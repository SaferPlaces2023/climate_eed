...
import pytest
import xarray as xr
from climate_eed import fetch_var_planetary, fetch_var_smhi, fetch_var_copernicus, list_repo_vars


def test_era5_fetch_var():
    """Test the fetch_var_planetary function."""
...
def test_ensemble_fetch_var_all():
    """Test the fetch_var_planetary function."""
    ...
    data_ensemble = fetch_var_planetary(varname=var_ensemble,
...
def test_ensemble_fetch_var_sel_model():
    """Test the fetch_var_planetary function with selected models."""
...
def test_seasonal_forecasts_fetch_var():
    ...
    print("----------------")
    assert type(data_seasonal_forecasts) == xr.Dataset
    assert data_seasonal_forecasts.sizes == {"id": 10, "time": 214, "model": 51}


def test_copernicus_fetch_var():
    """Test the fetch_var_copernicus function for Copernicus collection."""
    ...
    query = {
        ...
        'area': [
            41.73, 46.25, 41.73, 46.25],
    }    
    file_out = "./test_data/download_cds.nc"

    out = fetch_var_copernicus(dataset, query, file_out)
    assert type(out) == xr.Dataset
    assert out.sizes == {'longitude': 1, 'latitude': 1, 'number': 51, 'time': 860}